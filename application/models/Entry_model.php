...
class Entry_model extends CI_Model {
	...
	/*
	validasi data kelas full (kapasitas kelas)
	contoh :  10 orsng
	jika space masih 5 orang namun kres di waktu tetap di buang ke kelas (X,Y)

	 */
	public function kelas_full_kress($id_mk_tawaran) {
    ...
    from entry_temporary et
    join mk_tawaran mt on mt.id_mk_tawaran=et.id_mk_tawaran
    join jadwal_mk jm on mt.id_mk_tawaran=mt.id_mk_tawaran

    join matakuliah mk on mk.id_mk=mt.id_mk
    join kelas k on k.id_kelas=jm.id_kelas
	...
	public function join_mk_semester1($get,$mhs)
	{
		$this->db->join('mk_tawaran mt', 'mt.id_mk_tawaran = et.id_mk_tawaran');
		$this->db->join('kelas k', 'k.id_kelas = et.id_kelas');
		$this->db->join('jadwal_mk jmk', 'jmk.id_mk_tawaran = et.id_mk_tawaran');
	  ...
	  $this->db->join('semester s', 's.id_semester = mt.id_semester');
		$this->db->where('s.nama_semester', $get);
		$this->db->where('et.id_mahasiswa', $mhs);

	  // $this->db->order_by('s.nama_semester', 'asc');
	  return $this->db->get('entry et')->result();
	}



	public function join_mk_semester($mhs)
	{
		$sem_2 = 2;
		$sem_4 = 4;
		$sem_6 = 6;
		...
		// $this->db->order_by('s.nama_semester', 'asc');
		return $this->db->get('entry_temporary etmp')->result();
	}

 

	public function join_mk_semester3($mhs)
	{
