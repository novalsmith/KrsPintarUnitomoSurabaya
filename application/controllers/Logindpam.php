...
class Logindpam extends CI_Controller {
...
$this->load->view('login/logindpam', $data);
...
}else
{
    ...
    $hasil = $this->login_model->cek_dpam($data);
    ...
    if ($hasil->num_rows() == 1) 
    {
      foreach ($hasil->result() as $sess) {
        ...
        $sess_data['nama_dpam'] = $sess->nama_dpam;
 ...
 <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
     ...
     // echo "<script>alert('Gagal login: Cek username, password!');history.go(-1);</script>";
       ...
       public function logout() {
        $this->session->unset_userdata('nama_dpam');
